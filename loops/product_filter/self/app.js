...
},
{
  ...
  const filterButtons = document.querySelector('#filter_btns')
...
//    console.log(categories);
   ...
   categories.forEach((cat)=> {
      ...
      filter_btn.innerText = cat
      filter_btn.addEventListener("click",()=> {
          if (cat==="All") {
              renderProducts(products)
          } else {
             const query = products.filter(p => p.category === cat)
             renderProducts(query)
          }
      })

      filterButtons.appendChild(filter_btn)
