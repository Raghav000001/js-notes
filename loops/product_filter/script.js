...
const productCard = document.getElementById('product_list');
...
categories.forEach((c)=> {
    ...
    btn.addEventListener("click",()=> {
        if (c==="All") {
            ...
            const filtered = products.filter(Prod => Prod.category === c)
            showProducts(filtered)
            // console.log("Clicked:", c, "=> Filtered:", filtered);
        }
    })
...
searchBox.addEventListener("input", () => {
  ...
  const searchedProducts = currentProducts.filter(p =>
